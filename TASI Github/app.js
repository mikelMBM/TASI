...
var UebungenRouter = require('./routes/uebungen');
var TrainingsRouter = require('./routes/training');
var PlansRouter = require('./routes/plans');
var UsersRouter = require('./routes/users');
var TrackingRouter = require('./routes/tracking');

app.use('/', IndexRouter);
app.use('/', UebungenRouter);
app.use('/', TrainingsRouter);
app.use('/', PlansRouter);
app.use('/', UsersRouter);
app.use('/', TrackingRouter);

/* Auskommentierter Code wurde 2019 in die entsprechenden Routes ausgelagert ./routes
...
/*	New workout (table not available at the moment)
	if(req.body.startWorkout){
		squery = "INSERT INTO workout (NutzerID) VALUES ?";
		values = [[req.session.roleid]];
		connection.query(squery, [values], function(err, result) {
				if(err){
					console.log(err);	
				}else{
					session.workoutID = result.insertId;
	...
	if(req.body.uebungsattribut){	
		...
		for(var key in req.body) {
		  if(req.body.hasOwnProperty(key)){
		  	  ...
		  	  if(key==2){
		  	  	  console.log("Einsch�tzung: " + req.body[key]);
		  	  	  squery = "INSERT INTO achievements (UebungsID, UebungsattributID, NutzerID, Datum, Wert) VALUES ?";
		  	  	  values = [[session.uebungsID, key, req.session.roleid, date, req.body[key]]];
		  	  	  connection.query(squery, [values], function(err, result) {
		  	  	  		  if(err){
